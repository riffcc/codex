...
See the Codex keymap documentation for supported actions and examples."
        ...
        if let Some(started) = initial_started_thread {
            ...
            let cwd = started.session.cwd.to_path_buf();
            app.enqueue_primary_thread_session(started.session, started.turns)
                .await?;
            app.project_navigation
                .remember_project_thread(thread_id, cwd.clone());
            app.project_navigation.record_recent(cwd);
            app.project_navigation.set_active_thread(thread_id);
            app.sync_project_tabs_chrome();
        ...
        if self.overlay.is_some() {
            ...
            match event {
                ...
                TuiEvent::Gamepad(action) => {
                    self.handle_gamepad_action(tui, app_server, action).await;
                }
    ...
    fn render_chat_widget_frame(
        ...
        let project_tabs_line = self.chat_widget.project_tabs_line(terminal_width);
        let top_bar_height = u16::from(project_tabs_line.is_some());
        let desired_height = if self.split_pane.is_some() {
            split_pane::SplitPaneState::<ThreadId>::desired_height()
        } else {
            self.chat_widget.desired_height(terminal_width)
        }
        .saturating_add(top_bar_height);
        let mut rendered_area = Rect::default();
        let mut render = |frame: &mut crate::custom_terminal::Frame| {
            let frame_area = frame.area();
            let chat_area = if let Some(line) = project_tabs_line.clone() {
            ...
            if let Some(split_pane) = self.split_pane.as_ref() {
                ...
                split_pane.visit_leaf_areas(chat_area, &mut |pane_id, thread_id, pane_area| {
                    if pane_id == active_pane_id {
                        self.chat_widget.render(pane_area, frame.buffer);
                    } else {
                        let label = self
                            .project_navigation
                            .tab_cwd(thread_id)
                            .map(Self::project_label)
                            .unwrap_or_else(|| thread_id.to_string());
                        Paragraph::new(label)
                            .block(Block::default().borders(Borders::ALL))
                            .render(pane_area, frame.buffer);
                    }
                });
                split_pane.render_dividers(chat_area, frame.buffer);
                if let Some(active_area) = split_pane.active_pane_area(chat_area)
                    && let Some((x, y)) = self.chat_widget.cursor_pos(active_area)
                {
                    frame.set_cursor_style(self.chat_widget.cursor_style(active_area));
        ...
        if terminal_resize_reflow_enabled {
            tui.draw_with_resize_reflow(desired_height, &mut render)?;
        } else {
            tui.draw(desired_height, render)?;
        }
        Ok(rendered_area)
    }
}
