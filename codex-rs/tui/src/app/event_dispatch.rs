    ...
    pub(super) async fn handle_event(
        ...
        match event {
            ...
            AppEvent::FocusOrOpenProject { cwd } => {
                self.focus_or_open_project(tui, app_server, cwd).await?;
            }
            AppEvent::FocusOrOpenProjectAtTarget { cwd, target } => {
                self.focus_or_open_project_at_target(tui, app_server, cwd, target)
                    .await?;
            }
